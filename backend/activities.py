...
from classes.agents import CreditReportAgent, DataFetchAgent
    ...
    try:
        while not task.done():
            try:
                ...
                activity.logger.debug("heartbeat")
...
async def trigger_document_processing(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        activity.logger.info(
            f"Processing {doc_type} with AWS Bedrock Nova Pro (Strands): {local_path}"
        )
        ...
        try:
            ...
            activity.logger.warning(
                f"Unknown document type {doc_type}, using generic OCR prompt"
            )
        ...
        if file_ext not in supported_formats:
            raise ValueError(
                f"Unsupported file format: {file_ext}. Only image files are supported: {', '.join(supported_formats)}"
            )
        ...
        activity.logger.info(
            f"Invoking AWS Bedrock Nova Pro via Strands with model: {model_id}"
        )
        ...
        image_format = (
            file_ext if file_ext in ["png", "jpeg", "gif", "webp"] else "jpeg"
        )
        if file_ext == "jpg":
            image_format = "jpeg"
...
async def income_assessment(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        activity.logger.info(
            "Starting income assessment with AgentCore Code Interpreter"
        )
        ...
        if docs and isinstance(docs, dict):
            ...
            for doc in processed_docs:
                if (
                    doc.get("doc_type") == "bank_statement"
        ...
        bank_balance = (
            bank.get("accounts", [{}])[0].get("balance", 0)
            if bank.get("accounts")
            else 0
        )
        account_type = (
            bank.get("accounts", [{}])[0].get("type", "N/A")
            if bank.get("accounts")
            else "N/A"
        )
        ...
        activity.logger.info(
            "Invoking AgentCore Code Interpreter for income analysis..."
        )
        ...
        response = await run_blocking_with_heartbeats(
            agent, analysis_prompt, heartbeat_interval=60
        )

        # Extract response
        response_text = (
            str(response.message["content"][0]["text"])
            if response.message
            else "No response"
        )
...
async def expense_assessment(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        activity.logger.info(
            "Starting expense assessment with AgentCore Code Interpreter"
        )
        ...
        if docs and isinstance(docs, dict):
            ...
            for doc in processed_docs:
                if (
                    ...
                    bank_statement_data = doc.get("extracted_data", {})
                    activity.logger.info("Found bank statement for expense analysis")
                    break
        ...
        # Execute analysis with AgentCore Code Interpreter (run in executor)
        activity.logger.info(
            "Invoking AgentCore Code Interpreter for expense behavior analysis..."
        )
...
async def aggregate_and_decide(payload: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        raise ApplicationError(
            f"LLM call failed: {str(e)}", type="LLMError", non_retryable=False
        )
    ...
    recommendation = (
        ...
        if llm_error
            ...
            "approve"
            if payload.get("credit", {}).get("score", 600) > 650
            else "manual_review"
        )
    )
